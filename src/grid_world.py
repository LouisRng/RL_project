...
class GridWorld():
    ...
    def __init__(self, env_size=args.env_size, 
        ...
        self.reward_hit_wall = args.reward_wall
    ...
    def _get_next_state_and_reward(self, state, action):
        ...
        # useful to check the number of times each state is visited
        # if (x, y) == self.target_state:  # stay
        #     x, y = state
    ...
    def render(self, animation_interval=args.animation_interval):
        ...
        # set_data: x: ArrayLike, y: ArrayLike
        traj_x, traj_y = zip(*self.traj) # unzip into x and y coordinates         
    ...
    def save_graphics(self, filename='grid_world.png'):
        if self.canvas is not None:
            self.canvas.savefig(filename, dpi=300)
